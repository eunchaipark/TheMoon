...
def get_category_prefs(user_id: int) -> list[dict]:
    ...
    try:
        ...
        rows = cur.fetchall()
        cols = [desc[0] for desc in cur.description]
        return [dict(zip(cols, rows)) for row in rows]
    finally:
        conn.close()


def upsert_category_pref(user_id: int, prefs:list[dict]) -> None:
    conn = get_connection()
    try:
        cur = conn.cursor()
        for pref in prefs:
            cur.execute("""
                        INSERT INTO categories_prefs (user_id, category_id, weight)
                        VALUES (%s, %s, %s)
                        on conflict (user_id, category_id) DO UPDATE SET weight = EXCLUDED.weight
